...
async function fetchLatestMetricByPatterns(patterns) {
    ...
    try {
        const sql = `
            ...
            LIMIT 1
        `;
        const values = patterns.map((p) => p.toLowerCase());
        const [rows] = await pool.query(sql, values);
        return rows[0] || null;
    } catch (error) {
        console.warn('⚠️ Metric query fallback (DB indisponible?):', error.code || error.message);
        return null;
    }
}
...
async function fetchLatestTextByPatterns(patterns) {
    ...
    try {
        ...
        console.warn('⚠️ Metric texte fallback (DB indisponible?):', error.code || error.message);
        return null;
    }
}
...
async function fetchHistoryByPatterns(patterns, hours = 24, limit = 120) {
    ...
    try {
        ...
        return rows
            .reverse()
            .map((row) => ({ t: row.created_at, v: parseNumeric(row.value) }));
    } catch (error) {
        console.warn('⚠️ History query fallback (DB indisponible?):', error.code || error.message);
        return [];
    }
}
...
async function buildOverviewData() {
    ...
    const publicData = {
        ...
        salle1: {
            ...
            updated_at: metrics.salle1.temperature?.created_at || metrics.salle1.humidite?.created_at || metrics.salle1.lumiere?.created_at || null
        },
        salle2: {
            ...
            updated_at: metrics.salle2.temperature?.created_at || metrics.salle2.humidite?.created_at || metrics.salle2.lumiere?.created_at || metrics.salle2.pm25?.created_at || metrics.salle2.pm10?.created_at || null
        }
    };
    ...
    try {
        ...
        if (room === 'salle1') {
            ...
            return res.json({
                room: 'salle1',
                hours,
                series: { temperature, humidite, lumiere }
            });
        }
        ...
        if (room === 'salle2') {
            ...
            return res.json({
                room: 'salle2',
                hours,
                series: { temperature, humidite, lumiere, co2, air }
            });
        }
        ...
        return res.json({
            ...
            hours,
            series: {
                salle1Temperature: salle1Temp,
                salle2Temperature: salle2Temp
            }
        });
    } catch (error) {
        console.error('Erreur history public:', error);
        res.status(500).json({ error: 'Impossible de récupérer l’historique public' });
    }
});
...
app.get('/api/admin/overview', authAdmin, async (req, res) => {
    try {
        ...
        res.status(500).json({ error: 'Impossible de récupérer les métriques admin' });
    }
});
...
app.get('/api/admin/commands', authAdmin, (req, res) => {
    res.json({
        ...
        items: commandLogs.slice(-50).reverse()
    });
});
...
app.post('/api/admin/command', authAdmin, (req, res) => {
    ...
    if (!publishTopic) {
        return res.status(400).json({ error: 'Topic MQTT requis' });
    }

    if (!mqttClient.connected) {
        return res.status(503).json({ error: 'Broker MQTT non connecté' });
    }

    mqttClient.publish(publishTopic, publishPayload, { qos: 0 }, (err) => {
        if (err) {
            console.error('Erreur publish MQTT:', err);
            return res.status(500).json({ error: 'Échec publication MQTT' });
        }
